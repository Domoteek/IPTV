...
function installCMSPackages {
	...
	if [ "$DISTRIB_CODENAME" != "trusty" ]; then
		LANG=en_US.UTF-8 DEBIAN_FRONTEND=noninteractive apt-get install postgresql-9.4 -y --force-yes >> /dev/null 2>&1
...
function setupCMS {
	CHKZEND=`cat /etc/php5/apache2/php.ini | grep zend_exten`
        if [ -z "$CHKZEND" ]; then
                echo "zend_extension=/opt/fulliptv/lib/ioncube/ioncube_loader_lin_5.6.so" >> /etc/php5/apache2/php.ini
        fi
        CHKZEND=`cat /etc/php5/cli/php.ini | grep zend_exten`
        if [ -z "$CHKZEND" ]; then
                echo "zend_extension=/opt/fulliptv/lib/ioncube/ioncube_loader_lin_5.6.so" >> /etc/php5/cli/php.ini
        fi
